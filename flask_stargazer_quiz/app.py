...
from utils.calc_utils.trans_spheric2ortho import convert_star, convert_constline
from utils.calc_utils.remove_rows_by_criteria import remove_rows_by_criteria, remove_two_rows_by_criteria
...
class StarGazerApp(Flask):
    def __init__(self, *args, **kwargs):
        global star_loc_data, star_info_data, const_line_loc_data
...
def default_star_map():
    if request.method == 'GET':
        ...
        ortho_star_data, ortho_const_line = convert_star(star_loc_data), convert_constline(const_line_loc_data)
        filtered_star_data, filtered_const_line = remove_rows_by_criteria(ortho_star_data), remove_two_rows_by_criteria(ortho_const_line)
        print(filtered_const_line.shape)

        # generate star image
        generate_star_image(img_path, filtered_star_data, filtered_const_line)
